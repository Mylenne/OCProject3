...
class Magus: Character {
    ...
    init() {
        super.init(name: "", lifePoint: 50, weapon: "magic", destructionCapacity: 0, healingCapacity: 30)
    }
}
...
func chance (attacker: Character) {
    var newDestructionCapacity: Int = 0 // Crée une variable qui contiendrais le choix de l'attackant et lui rajouté.destructionCapacity
    // Add new weapon to a character ex: choosenCharacter.weapon
    var newWeapon: String = ""
    
    
    if !(attacker is Magus) {
        let randomAttackerWeapon: [String: Int] = ["Dagger": -5, "Nunchuku": -10, "Sai": -15, "Maul": -20, "Halberd": -40, "Bow and arrow": -25]
        // Search a random number in randomAttackerWeapon
        let randomNumber = arc4random_uniform(UInt32(randomAttackerWeapon.count))
        // Obtenir les clés "string" du tableau randomAttackerWeapon puis lui mettre un numero pour tombé sur un des elements
        let keyAttackerWeapon = Array(randomAttackerWeapon.keys)[Int(randomNumber)]
        // exemple randomAttackerWeapon[0] = "dagger"
        if attacker.lifePoint == 0 {
            newDestructionCapacity = 0
            print("Your character is already dead he won't get any new power")
        } else {
            newDestructionCapacity = (randomAttackerWeapon[(keyAttackerWeapon)])! // Crée une variable qui contiendrais le choix de l'attackant et lui rajouté.destructionCapacity
            // Add new weapon to a character ex: choosenCharacter.weapon
        }
        newWeapon = keyAttackerWeapon
        
    } else {
        
        let randomMagusWeapon = ["Wand": +35, "Healing sword": +40 ,"Healing dust": +15, "Potion": +10, "Life liquor": +20, "Magic hat": +25]
        // Search a random number in randomAttackerWeapon
        let randomNumber = arc4random_uniform(UInt32(randomMagusWeapon.count))
        // Obtenir les clés "string" du tableau randomAttackerWeapon puis lui mettre un numero pour tombé sur un des elements
        let keyMagusHealingWeapon = Array(randomMagusWeapon.keys)[Int(randomNumber)]
        newDestructionCapacity = randomMagusWeapon[keyMagusHealingWeapon]!
        newWeapon = keyMagusHealingWeapon
        
    }
    
    attacker.destructionCapacity = newDestructionCapacity //coffre
    attacker.weapon = newWeapon
    print("The character has found a box and has changed his old weapon to a \(attacker.weapon) that he just found and has now the capacity to take away \(attacker.destructionCapacity) point from his opponent")
}
